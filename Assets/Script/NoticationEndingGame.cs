...
public class NoticationEndingGame : MonoBehaviour
{
    ...
    /// <summary>
    /// **MỚI: Hiển thị message 3 (graduation) với GPA**
    /// </summary>
    public void GetMes3WithGPA(float gpa)
    {
        ...
        // **SỬA: Kích hoạt GameObject của textGPA (không chỉ component)**
        if (textGPA != null)
        {
            ...
            textGPA.text = $"Chúc mừng bạn đã hoàn thành chặng đường của mình trong suốt 5 năm qua. GPA bạn đạt được là: {gpa} / 4.0";
            
            // Tô màu theo ranking
            if (gpa >= 3.6f)
            {
                textGPA.color = new Color(1f, 0.84f, 0f); // Xuất sắc - Vàng
            }
            else if (gpa >= 3.2f)
            {
                textGPA.color = new Color(0f, 1f, 0f); // Giỏi - Xanh lá
            }
            else if (gpa >= 2.5f)
            {
                textGPA.color = new Color(0f, 0.7f, 1f); // Khá - Xanh dương
            }
            else if (gpa >= 2.0f)
            {
                textGPA.color = new Color(1f, 0.65f, 0f); // Trung bình - Cam
            }
            else
            {
                textGPA.color = new Color(1f, 0.2f, 0.2f); // Yếu - Đỏ
            }
            
            Debug.Log($"[NoticationEndingGame] Graduation message displayed with GPA: {gpa:F2}, GameObject active: {textGPA.gameObject.activeSelf}");
        }
        else
        {
